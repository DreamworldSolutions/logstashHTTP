...
function logstashHTTPAppender(config) {
  ...
  const agent = config.url.startsWith('http://') ? new http.Agent(config.agentOptions) : new https.Agent(config.agentOptions);
  ...
  return function log(event) {
    ...
    // send to server
    sender.post('', logstashJSON, { httpAgent: agent, httpsAgent: agent })
      .catch((error) => {
        if (error.response) {
